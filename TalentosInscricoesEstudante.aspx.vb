...
Partial Class SiteTalentos_TalentosInscricoesEstudante
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            If oDt.Rows.Count > 0 Then
                LitInscricao.Text = "<h3 class='text-center text-black'>Inscrições abertas para o " & oDt.Rows(0).Item("TalTitulo") & "</h3> <a href='TalentosInscricaoEstudante' class='btn btn-dark float-right'>Nova Inscrição</a>"
            Else
...
#Region "Busca"
    ...
    Private Sub BuscarInscricao()
        Dim oDt As Data.DataTable
        Dim Consulta As New ClsConsulta
        oDt = Consulta.ConsultaSQL("select InsCodigo, TalTitulo, InsDataCadastro, InsDataAlteracao, InsDataFinalizacao, InsTitulo, InsNomeEquipe, InsCoordenador, InsResponsavel, InsFinalizado from TalentosInscricaoEstudante inner join TalentosAgencia on InsAgeCodigo = AgeCodigo inner join TalentosNovo on InsTalCodigo = TalCodigo where InsAgeCodigo = " & HidAgeCodigo.Value & " order by InsDataCadastro desc")
